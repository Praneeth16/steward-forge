...
The installed Databricks CLI schema recognizes the experiment resource and App experiment binding used here. The updated bundle has not completed authenticated validation or deployment: the selected local profile's refresh token expired before the request visitor could resolve workspace values. A deployer must run both default and alternate endpoint validation after authenticating, then install a real scoped trace adapter and provider transport before enabling model calls.
